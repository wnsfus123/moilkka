...
module.exports = async (req, res) => {
  ...
  // ── PATCH: 예약 페이지 수정 ─────────────────────────────────────
  if (req.method === 'PATCH') {
    const patchUuid = uuid;
    const { kakao_id, title, description } = req.body;
    if (!patchUuid) return res.status(400).json({ error: 'uuid 필요' });
    if (!kakao_id || !title) return res.status(400).json({ error: '필수 값 누락' });
    try {
      const { data: pg } = await supabase.from('booking_pages').select('kakao_id').eq('uuid', patchUuid).single();
      if (!pg) return res.status(404).json({ error: '페이지를 찾을 수 없습니다.' });
      if (String(pg.kakao_id) !== String(kakao_id)) return res.status(403).json({ error: '권한 없음' });
      const { error } = await supabase.from('booking_pages')
